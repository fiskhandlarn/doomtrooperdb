...
(function ui_decklist_search(ui, $) {
  ...
  ui.setup_typeahead = function setup_typeahead()
  {
    ...
    $('#card').on('typeahead:selected typeahead:autocompleted', function (event, data) {
      ...
      var line = $(
        '<p class="fg-' + card.faction_code + '" style="padding: 3px 5px;border-radius: 3px;border: 1px solid silver"><button type="button" class="close" aria-hidden="true">&times;</button><input type="hidden" name="cards[]" value="' + card.code + '">' +
          card.label + '</p>'
      );
      line.on({
